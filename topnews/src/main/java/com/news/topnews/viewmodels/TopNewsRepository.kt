...
class TopNewsRepository @Inject constructor(
    ...
    /**
     *   Api Call for Top News List
     *   viewModelScoped is for calling this coroutine in the viewmodel scope
     *   params: is the query parameter which api required
     */
    fun getTopNews(viewModelScoped: CoroutineScope, params: Map<String, String>) {
        viewModelScoped.launch(Dispatchers.IO) {

            val newsList = newsDataBaseRepository.getData(isTopNews = true)
            ...
            kotlin.runCatching {
                val response = mApiClient.getTopNews(params)
                response.await().let {
                    if (it.status == BaseConstants.ApiConstant.API_SUCCESS) {
                        newsDataBaseRepository.inserData(it.articles, isTopNews = true)
